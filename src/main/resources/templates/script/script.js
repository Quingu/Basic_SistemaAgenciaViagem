...
async function exibirPacotes(pacotes) {
    ...
    // Usamos Promise.all para carregar todas as imagens de forma assíncrona
    await Promise.all(pacotes.map(async (pacote) => {
        const imagePath = await getImagePath(pacote.destino);

        const card = document.createElement('div');
        card.className = 'pacote-card';
        card.innerHTML = `
            ${pacote.preco < 4000 ? '<span class="destaque-tag">Oferta!</span>' : ''}
            <img src="${imagePath}" alt="${pacote.destino}" class="pacote-imagem">
